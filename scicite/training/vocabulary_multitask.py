...
class VocabularyMultitask(Registrable):
    """
    A Vocabulary maps strings to integers, allowing for strings to be mapped to an
    out-of-vocabulary token.
    ...
    def from_params(cls, params: Params, instances: Iterable['adi.Instance'] = None,
                    instances_aux: Optional[Iterable['adi.Instance']] = None):  # type: ignore
        """
        ...
        instances_aux: optional
            Instances for the auxiliary task
        ...
        if vocabulary_directory:
            vocab = VocabularyMultitask.from_files(vocabulary_directory)
            if not extend:
        ...
        if extend:
            ...
            if instances_aux is not None:
                raise NotImplementedError('vocab extension not implemented for multitask setting')
            return vocab
        ...
        return VocabularyMultitask.from_instances(instances=instances,
                                         min_count=min_count,
                                         max_vocab_size=max_vocab_size,
                                         non_padded_namespaces=non_padded_namespaces,
                                         pretrained_files=pretrained_files,
